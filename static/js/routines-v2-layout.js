(() => {
  ...
  const mkId = p => `${p}_${Math.random().toString(36).slice(2)}_${Date.now().toString(36)}`;
  const priority = name => {
    const order = ['brush','floss','shower','moisturize','vitamin','water','dishes','tidy','night routine'];
    const n = String(name||'').toLowerCase();
    const i = order.findIndex(x => n.includes(x));
    return i < 0 ? 50 : i;
  };
  ...
  function parseWeekdays(raw){ const map={mon:0,monday:0,tue:1,tues:1,tuesday:1,wed:2,weds:2,wednesday:2,thu:3,thur:3,thurs:3,thursday:3,fri:4,friday:4,sat:5,saturday:5,sun:6,sunday:6}; const out=[]; String(raw||'').split(/[,\s]+/).forEach(p=>{p=p.trim().toLowerCase(); if(!p)return; if(/^[0-6]$/.test(p))out.push(Number(p)); else if(map[p]!==undefined)out.push(map[p]);}); return [...new Set(out)].sort((a,b)=>a-b); }
  ...
  function item(s,r,today){
    ...
    if(r.repeat_type==='daily'){ if(tc.length){bucket='done'; rank=80; text='Done'; due=addDays(today,1);} else {bucket='today'; rank=10+priority(r.name)/10;} }
    else if(r.repeat_type==='times_per_day'){ const goal=Math.max(1,Number(r.repeat_value||1)), done=Math.min(tc.length,goal); text=`${done}/${goal}`; rank=10+priority(r.name)/10+done/100; if(done>=goal){bucket='done'; rank=80+priority(r.name)/10; text=`Done · ${done}/${goal}`; due=addDays(today,1);} else bucket='today'; }
  ...
  function complete(id){ const s=state(); s.completions.push({id:mkId('completion'), routine_id:id, completed_at:nowIso(), completed_date:todayIso()}); save(s); render(); }
  ...
  function syncForm(){ const type=$('typeInput').value; $('valueLabel').style.display=(type==='daily'||type==='weekdays')?'none':''; $('daysLabel').style.display=type==='weekdays'?'':'none'; }
  function openEdit(r){ const f=$('formWrap'); if(!f)return; f.classList.add('open'); $('editId').value=r?.id||''; $('formTitle').textContent=r?'Edit routine':'Add routine'; $('nameInput').value=r?.name||''; $('areaInput').value=r?.area||''; $('typeInput').value=r?.repeat_type||'every_n_days'; $('valueInput').value=r?.repeat_value||1; $('daysInput').value=(r?.days_of_week||[]).map(d=>weekdays[d]).join(', '); $('notesInput').value=r?.notes||''; $('deleteBtn').style.display=r?'':'none'; syncForm(); $('nameInput').focus(); }
  function closeForm(){ $('formWrap').classList.remove('open'); $('editId').value=''; }
  function saveForm(){ const s=state(), rid=$('editId').value, type=$('typeInput').value, name=$('nameInput').value.trim(); if(!name)return; const next={id:rid||mkId('routine'), name, area:$('areaInput').value.trim()||'General', repeat_type:type, repeat_value:(type==='daily'||type==='weekdays')?null:Math.max(1,Math.round(Number($('valueInput').value||1))), days_of_week:type==='weekdays'?parseWeekdays($('daysInput').value):[], notes:$('notesInput').value.trim(), active:true, updated_at:nowIso(), created_at:nowIso()}; if(type==='weekdays'&&!next.days_of_week.length)return; const i=s.routines.findIndex(r=>r.id===rid); if(i>=0)s.routines[i]={...s.routines[i],...next,created_at:s.routines[i].created_at||next.created_at}; else s.routines.push(next); save(s); closeForm(); render(); }
  function deleteCurrent(){ const rid=$('editId').value; if(!rid)return; const s=state(), r=s.routines.find(x=>x.id===rid); if(r){r.active=false;r.deleted=true;r.updated_at=nowIso();} save(s); closeForm(); render(); }
  function exportState(){ navigator.clipboard?.writeText(JSON.stringify(state(),null,2)); }
